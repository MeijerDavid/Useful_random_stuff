function S = structArray2struct(SA,dims)
%Convert structure-array to a simple struct(-array) with the same fields 
%but where each of the fields' arrays has been concatenated (into a new 
%dimension) along each of the struct-array dimensions specified by 'dims'.

%By default, concatenate across all dimensions of the structure-array
if nargin < 2; dims = 1:numel(size(SA)); end

%Convert structure-array to cell-array (first dimension = fields)
C = struct2cell(SA);

%Concatenate the cells' contents across each of the desired dimensions
for d=dims
    C = catCellContent(C,d+1);
end

%Return back from cell-array to structure-array
S = cell2struct(C,fieldnames(SA(1)),1);  
