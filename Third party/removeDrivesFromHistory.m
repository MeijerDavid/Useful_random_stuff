function removeDrivesFromHistory()
% removeDrivesFromHistory()
%   Alters current folder history by removing selected paths. This is necessary 
% to fix a common problem associated with network locations on the current folder 
% path history that are no longer available.  When an undefined function or variable 
% error is elicited, Matlab will search the current folder history and when it 
% encounters a path to an unavailable location, the program will stall for an 
% unbearable amount of time before throwing the error.  See the links below for 
% discussions on this matter in TMW forum.
%
% Before removing any history, a backup is saved to a text file.  It is overwritten 
% each time. The file is named pathHistoryBackup.txt and is saved in tempdir().
%
% removeDrivesFromHistory()
% A GUI will be generated that lists the paths that appear in the current folder history.  
% Select paths to be removed and press "Remove". The GUI has the following buttons:
%   * Show current (history): The current folder path history will appear in the 
%       command window.
%   * Show backup: The backup txt file will open showing the most recent backup.
%   * Restore history: Matlab's current folder history will be replaced with the 
%       backup file and the GUI is regenerated.
%   * Help: help doc will appear.
%   * Close: GUI closes; nothing happens.
%   * Remove: After overwriting the backup file with a new backup, the current folder 
%       history is changed and the GUI is regenerated.
%
% Discussion links
% https://www.mathworks.com/matlabcentral/answers/395876-undefined-function-error-is-very-slow-to-occur#answer_315892
% https://www.mathworks.com/matlabcentral/answers/364153-40-seconds-waiting-for-undefined-function-or-variable-error-2017b
%
% Adam Danz 08/2019
% Copyright (c) 2014, Adam Danz
% All rights reserved
% https://www.mathworks.com/matlabcentral/fileexchange/72519-removedrivesfromhistory
%
% Email: (Run the line below)
% char([97 100 97 109 46 100 97 110 122 64 103 109 97 105 108 46 99 111 109]) 

% Change history
% 191119 Fixed typo in the "Your current directory is..." message box. 

%% Prepare for the GUI
persistent initialMessage

% This solution is not applicable to releases prior to r2017b [1]
v = ver('Matlab');
