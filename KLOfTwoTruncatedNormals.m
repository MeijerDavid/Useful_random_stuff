function KLdiv = KLOfTwoTruncatedNormals(mu1,sd1,mu2,sd2,a,b)
...
if nargin < 5
    ...
    KLdiv = log(sd2./sd1) + (sd1.^2+(mu1-mu2).^2)./(2*sd2.^2) -.5;              %See Eq. 112 in Nielsen 2022, but there are many other sources 
    %KLdiv = ((mu1-mu2).^2 + sd1.^2 - sd2.^2) ./ (2*sd2.^2) + log(sd2./sd1);    %equivalent, see: http://allisons.org/ll/MML/KL/Normal/
    ...
    KLdiv = .5*(mu2.^2./var2 - mu1.^2./var1) + log((sd2.*normcdf_quick((a-mu2)./sd2,(b-mu2)./sd2))) - log((sd1.*Z_1)) - (mu2./var2 - mu1./var1).*eta1 - .5*(1./var1 - 1./var2).*eta2;  %Eq. 111 
