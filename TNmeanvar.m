...
if any([numel(a),numel(b),numel(mu),numel(sd)] > 1)
    ...
    for j=1:numel(a)
        if nargout == 1
            m(j) = TNmeanvar(a(j),b(j),mu(j),sd(j));
        elseif nargout == 2
            [m(j),v(j)] = TNmeanvar(a(j),b(j),mu(j),sd(j));
        end
    end
...
%Default behaviour    
    ...
    if (a <= 0) && (0 <= b)
        ...
        if nargout == 2
            ...
            assert((fb >= fa) && (eb >= ea),['a=' num2str(a) ', b=' num2str(b)]);
            assert((0 <= m2) && (m2 <= 1),['a=' num2str(a) ', b=' num2str(b)]);
        end
    ...
    elseif (0 < a) && (a < b)
        ...
        if nargout == 2
            ...
            assert((a^2 <= m2) && (m2 <= b^2),['a=' num2str(a) ', b=' num2str(b)]);
        end
    else
        error('invalid combination of a and b');
    end
    
    %minor correction for numerical issues on mean
    assert(~isinf(m) && ~isnan(m),['Mean of TN is inf or nan, a=' num2str(a) ', b=' num2str(b)]);
    m = min(max(m,a),b);
    ...
    if nargout == 2
        ...
        assert(~isinf(v) && ~isnan(v),['Variance of TN is inf or nan, a=' num2str(a) ', b=' num2str(b)]);
        v = min(max(v,0),1);    
